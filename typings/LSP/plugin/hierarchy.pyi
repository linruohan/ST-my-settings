...
from .core.tree_view import (
    TreeDataProvider as TreeDataProvider,
    TreeItem as TreeItem,
    new_tree_view_sheet as new_tree_view_sheet,
)
...
class HierarchyDataProvider(TreeDataProvider):
    ...
    def get_children(
        self, element: HierarchyItemWrapper | None
...
def make_data_provider(
    ...
    sheet_name: str,
    direction: int,
    response: list[HierarchyItemWrapper],
) -> HierarchyDataProvider: ...
def incoming_calls_handler(
    response: list[CallHierarchyIncomingCall] | None | Error,
) -> list[HierarchyItemWrapper]: ...
def outgoing_calls_handler(
    response: list[CallHierarchyOutgoingCall] | None | Error,
) -> list[HierarchyItemWrapper]: ...
def type_hierarchy_handler(
    response: list[TypeHierarchyItem] | None | Error,
) -> list[HierarchyItemWrapper]: ...
...
def open_first(
    window: sublime.Window, session_name: str, items: list[HierarchyItemWrapper]
) -> None: ...
